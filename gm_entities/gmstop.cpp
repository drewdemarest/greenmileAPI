...
GMStop::GMStop()
{
    ...
    objDouble["arrivalLatitude"]                = objDoubleInit;
    objDouble["arrivalLongitude"]               = objDoubleInit;
    objDouble["serviceLatitude"]                = objDoubleInit;
    objDouble["serviceLongitude"]               = objDoubleInit;
    objDouble["baseLineService"]                = objDoubleInit;
    objDouble["plannedService"]                 = objDoubleInit;
    objDouble["projectedService"]               = objDoubleInit;
    objDouble["actualService"]                  = objDoubleInit;
...
QJsonObject GMStop::exportEngine()
{
    ...
    for(auto key : objDouble.keys())
        if(importedMember[key] && objDouble[key])
            json[key] = *objDouble[key];
    ...
    for(auto key : objGMStopType.keys())
        if(importedMember[key] && objGMStopType[key])
            json[key] = objGMStopType[key]->exportJson();

    for(auto key : objGMUdfs.keys())
        if(importedMember[key] && objGMUdfs[key])
            json[key] = objGMUdfs[key]->exportJson();

    for(auto key : objGMStop.keys())
        if(importedMember[key] && objGMStop[key])
            json[key] = objGMStop[key]->exportJson();
...
{  
        ...
        if(importedMember[key])
        {
            if(objQDateTime[key])
                *objQDateTime[key] = QDateTime::fromString
                ...
                objQDateTime[key] = new QDateTime
                        (QDateTime::fromString(json[key].toString(), Qt::ISODateWithMs));
        }
    }
    ...
    for(auto key : objGMRoute.keys())
    {
        ...
        {
            if(objGMRoute[key])
                objGMRoute[key]->importJson(json[key].toObject());
            else
            {
                objGMRoute[key] = new GMRoute();
                objGMRoute[key]->importJson(json[key].toObject());
            }
        }
    }
    ...
    for(auto key : objGMStopType.keys())
    {
        if(importedMember[key])
        {
            if(objGMStopType[key])
                objGMStopType[key]->importJson(json[key].toObject());
            else
            {
                objGMStopType[key] = new GMStopType();
                objGMStopType[key]->importJson(json[key].toObject());
            }
        }
    }
    ...
    for(auto key : objGMUdfs.keys())
    {
        ...
        {
            if(objGMUdfs[key])
                objGMUdfs[key]->importJson(json[key].toObject());
            else
            {
                objGMUdfs[key] = new GMUdfs();
                objGMUdfs[key]->importJson(json[key].toObject());
            }
        }
    }
    ...
    for(auto key : objGMStop.keys())
    {
        ...
        {
            if(objGMStop[key])
                objGMStop[key]->importJson(json[key].toObject());
            else
            {
                objGMStop[key] = new GMStop();
                objGMStop[key]->importJson(json[key].toObject());
            }
        }
    }
